...
final class PersistedConversation {
    ...
    var lastRawRequestDetails: String?
...
extension PersistedConversation {
    ...
    convenience init(from conversation: Conversation) {
        self.init(
            ...
            lastRawRequestDetails: conversation.lastRawRequestDetails
        )
    ...
    func toVolatile() -> Conversation {
        Conversation(
            id: id,
            title: title,
            messages: messages.map { $0.toVolatile() }.sorted(by: { $0.timestamp < $1.timestamp }),
            timestamp: timestamp,
            presetId: presetId,
            compactedSummary: compactedSummary,
            lastRawRequestDetails: lastRawRequestDetails
        )
    }
}
